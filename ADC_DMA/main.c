...
volatile uint16_t sg1_numb, sg2_numb, sg3_numb, sg4_numb;
uint16_t licznik = 0;
uint8_t display = 1;
...
void display_select()
{
	sg1_numb = licznik/1000;
	sg2_numb = (licznik/100) % 10;
    sg3_numb = (licznik / 10) % 10;
    sg4_numb = licznik % 10;
...
{
	ADC_Values[0] = 3.3 * ADC_Values[0] /4.095;
	licznik = ADC_Values[0];
	TxSize = snprintf(buf_Tx, sizeof(buf_Tx), "%d,%d\n", ADC_Values[0], ADC_Values[1]);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, S7G_Pin|S7D_Pin|S7E_Pin|S7C_Pin
                          |S7B_Pin|S7F_Pin|S7A_Pin|S7Dp_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOC, S7Com4_Pin|S7Com3_Pin|S7Com2_Pin|S7Com1_Pin, GPIO_PIN_SET);
